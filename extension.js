...
function activate(context) {
	...
	const openOutputCommand = vscode.commands.registerCommand('extension.tortor.openOutput', () => {
		outputChannel.show();
	});
	context.subscriptions.push(openOutputCommand);
	statusBarItem.command = 'extension.tortor.openOutput';

	let child;

	const tortorCommand = vscode.commands.registerCommand('extension.tortor', () => {
		vscode.window.withProgress({
			...
			try {
				...
				child.on('close', (code, kill) => {
					statusBarItem.show();
					if (code === 0) {
						statusBarItem.text = 'Tortor: Success';
						statusBarItem.backgroundColor = undefined;
					} else if (kill) {
						statusBarItem.text = 'Tortor: Killed';
						statusBarItem.backgroundColor = undefined;
					} else {
						statusBarItem.text = `Tortor: Error (${code})`;
	...
	context.subscriptions.push(tortorCommand);
}
